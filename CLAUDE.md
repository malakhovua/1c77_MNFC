...
**Коли пишеш щось з нуля** (нову процедуру/функцію повністю, новий модуль): коментар `// + claude` ставиш лише на найвищому рівні ієрархії того, що створив — рядок процедури/функції (або модуля, якщо новий увесь модуль). У внутрішніх умовних блоках, циклах тощо всередині цієї нової процедури коментар **не** дублюється.
Ієрархія: Весь модуль → Процедура → умовні блоки/цикли і т.д. — мітка ставиться тільки на верхньому рівні того, що додано.

Коли ж редагуєш/фіксиш окремі існуючі рядки (а не пишеш блок з нуля) — мітку `// + claude` ставиш саме на змінених рядках, як і раніше.
